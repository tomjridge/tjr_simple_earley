...
from `(i,X)` to a set of nonterminal items. _In the implementation we
distinguish whether the items are blocked at the current stage
`B(k,X)` or some previous stage `B(k'<k,X)`._
...
have already attempted to parse `T` at stage k._
...
`done`, in order to avoid adding an item more than once to `todo`._
...
* If item of the form `X -> i,as,k,[]` then process complete item
  ...
  `(i,X,k)` and only process each once._
...
* If item of the form `X -> i,as,k,Y bs` then process `Y` at stage
  ...
  implementation we only expand `Y` once at stage `k`._
