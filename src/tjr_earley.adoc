...
with an input "111..." of length 400, the parse takes about 3.5s on a
...
using "horrible" grammars like the above, and long inputs. This code
is meant to be a reference implementation for Earley-like parsing.
...
latexmath:[k]. The existence of an item at stage latexmath:[k] means
...
The type for (nonterminal) items, together with associated
operations. An item is like a record, of the form `{nt;i;as;k;bs}`,
indicating that the sequence of symbols `as` could be parsed between
`i` and `k`; this corresponds to an attempt to parse the production
`nt -> as bs` starting at position `i`).
...
* triples `(i,X,k)` of a symbol `X` and two integers `i,k`, which
...
If it is complete, we have a complete item `(i,X,k)` which we may have
seen before or not. We check whether it is `already_done`.
...
need to process against items blocked on `k,T`. In fact, we only need
to worry about the current blocked item (FIXME what was the reason?),
which we cut against each of the `j`s to get new todo items.
