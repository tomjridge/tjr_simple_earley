...
def nitm(nt:String,i:String,as:String,k:String,bs:String) = {
  s"""latexmath:[( $nt \\rightarrow {}_{$i} $as {}_{$k} . $bs)]"""
}

def nitm_trad (nt:String,i:String,as:String,k:String,bs:String) = {
  s"""latexmath:[( $nt \\rightarrow $as . $bs, $i, $k)]"""
}
...
val adoc2 = adoc.lines.map(s => 
  if (s.startsWith("//adoc ")) s.substring(7) 
  else if (s.startsWith("//ml ")) s.substring(5)
  else s).mkString("\n")

// write doc to README.scala.adoc

import java.io._

new PrintWriter("e_cn_inline_comments.pp.adoc") { write(adoc2); close }